...
import 'package:motoboy/features/chat/domain/models/message_model.dart';
import 'package:motoboy/features/help_and_support/domain/models/predefined_faq_model.dart';
import 'package:motoboy/features/help_and_support/domain/services/help_and_support_service_interface.dart';
import 'package:motoboy/features/help_and_support/screens/support_chat_screen.dart';
...
class HelpAndSupportController extends GetxController implements GetxService{
  ...
  void downloadFile({required String url, required String fileName}) async {
    ...
    final Directory downloadDirectory = Directory('${baseDirectory.path}/downloads');
    if (!await downloadDirectory.exists()) {
      await downloadDirectory.create(recursive: true);
    }

    final String filePathName = "${downloadDirectory.path}/$fileName";
    ...
    if(GetPlatform.isIOS){
      ...
      if (apiResponse.statusCode == 200) {
        ...
        apiResponse.listen((d) => downloadData.addAll(d), onDone: () async {
